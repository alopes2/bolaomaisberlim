...
export class ApiClient implements ProfileApi, AdminApi {
  ...
  async getPublicPredictions(matchId: string) {
    ...
    if (!response.ok) throw new Error('Os palpites ainda não estão disponíveis.')
  ...
  async getAdminResult(matchId: string) {
    const response = await this.authorizedFetch(`/admin/matches/${matchId}/raw-result`)
    if (!response.ok) throw new Error('Não foi possível carregar o resultado provisório.')
  ...
  async getProvisionalLeaderboard(matchId: string) {
    ...
    if (!response.ok) throw new Error('Não foi possível carregar o ranking provisório.')
    return (await response.json()) as LeaderboardResponse
  }

  async saveAdminResult(matchId: string, result: AdminResultResponse) {
    const response = await this.authorizedFetch(`/admin/matches/${matchId}/result`, {
      method: 'PUT',
  ...
  async confirmResult(matchId: string) {
    const response = await this.authorizedFetch(`/admin/matches/${matchId}/confirm`, {
      method: 'POST',
    })
    if (!response.ok) throw new Error('Não foi possível confirmar o resultado.')
  }
